...
def scale_sequence(
    ...
    """Derives a numeric sequence by scaling a known sequence against a specific value.

    The function creates a scaling factor such that the value of index ``offset`` in the returned sequence
    is equal to ``value``. All other indexes are scaled according to this scaling factor.

    If the sequence's value at the given index is ``0``, a scaling factor of ``1`` will be used.
    ...
    Args:
        ...
        offset (optional): The index within the sequence where ``value`` should be after scaling. If this
            value is not given or is outside the bounds of the sequence, the index nearest the midpoint of
...
def ratio_sequence(seed: Numeric, ratio: Numeric, count: int) -> Sequence[Numeric]:
    ...
    Items before ``seed`` in the resulting sequence will be successively divided by ``ratio``, and those
    after ``seed`` will be successively multiplied by ``ratio``.
    ...
    Args:
        seed: The value to build the sequence around. This value will be in the middle of the returned
            sequence.
        ratio: The ratio to apply to each element to find its predecessor (via division) or its successor
            (via multiplication).
        count: The number of items to be created before and after ``seed`` in the returned sequence. The
            length of the returned sequence will always be ``2 * count + 1``.
...
def round_sequence(
    ...
    """Rounds each number in a sequence, returning a new sequence of the rounded results.
    ...
    Args:
        ...
        digits (optional): An integer specifying the number of decimal places. If omitted, defaults to `0`.
...
def fmod_sequence(seq: Sequence[Numeric], divisor: Numeric) -> Sequence[Numeric]:
    """Applies ``fmod`` to each number in a sequence, returning a new tuple of the results.
