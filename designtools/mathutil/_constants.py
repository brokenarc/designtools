...
"""Two quantities are in the `golden ratio`_ if their ratio is the same as the ratio of their sum to
...
"""The `golden angle`_ (g) is the smaller of the two angles created by sectioning the circumference
of a circle according to the golden ratio; that is, into two arcs such that the ratio of the length
of the smaller arc to the length of the larger arc is the same as the ratio of the length of the
larger arc to the full circumference of the circle.
...
"""Two quantities are in the `supergolden ratio`_ if their quotient equals the unique real solution
to the equation ``x**3 == x**2 + 1``. This solution is commonly denoted ψ (psi).
...
"""Two quantities are in the `silver ratio`_ if the ratio of the smaller of those two quantities to
the larger quantity is the same as the ratio of the larger quantity to the sum of the smaller
quantity and twice the larger quantity.

The silver ratio is the limiting ratio of consecutive Pell numbers and is the second metallic mean.
It is denoted by δS (delta, subscript S).
...
"""The `plastic number`_ ρ (rho)  is a mathematical constant which is the unique real solution of
