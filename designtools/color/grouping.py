...
def group_colors(
    ...
    method that will check a Color instance for membership.
    ...
    if it occurs more than once in ``colors``.
    ...
    Returns:
        A mapping of group names to a sequence of Color instances that were
        collected by that group's membership tests. The keys in this group will
        be the same as the keys for ``collectors``. Note that the associated
        sequence may be empty of none of the collector for that key did not
